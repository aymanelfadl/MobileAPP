...
const AddSpendModalArticle = ({ visible, article, onClose }) => {
  ...
  const handleSpendAmount = async () => {
    ...
    try {
      setUploadProgress(0);
      const articleDoc = await firestore().collection('itemsCollection').doc(article.id).get();
      const currentSpends = articleDoc.data().spends;
      ...
      await firestore().collection('itemsCollection').doc(article.id).update({
        spends: totalSpends,
...
export default AddSpendModalArticle;
